...
def group_simulation(brackets, key, teams, eliminated, N=10000):
    ...
    for _ in range(N):
        ...
        ranks = {
            r: sorted(
                scores,
                key=lambda x: (scores[x][r], scores[x]['NCG']),
                reverse=True
            )
            for r in rounds
        }
...
def group_analysis(brackets, sim_results, key, eliminated, teams):
    ...
    current_scores = [
        {
            ...
            'Score': get_bracket_score(brackets[b], key, eliminated)[0],
            'Max': get_bracket_score(brackets[b], key, eliminated)[1],
        } for b in brackets
    ]
    current_scores = sorted(
        current_scores,
        key=lambda x: (x['Score'], x['Max']),
        reverse=True
    )
    ...
    upsets_chosen = sorted(
        ...
        key=lambda x: (
            x['Chosen'],
            int(x['Correct'].split('/')[0]) /
            max(int(x['Correct'].split('/')[1]), 1)
        ),
        reverse=True
    )
